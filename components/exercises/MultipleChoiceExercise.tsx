...
export default function MultipleChoiceExercise({
 ...
 // const { showPopUp, updatePopUp, hidePopUp } = usePopUp();
  ...
  const isCorrect = (answer: string | null) => {
    if(answer == null)
      return false;
    return answer.toLowerCase().trim() === content.correct_answer.toLowerCase().trim();
  };
...
{options.map((item, index) => (
         <li> <button style={{ 
          backgroundColor: selectedOption === item ? 'green' : 'black', color: 'white', padding: '10px 20px', border: 'none', borderRadius: '5px' }} 
          key={index}
...
return(
  ...
  <button
  style = {{ 
    backgroundColor:'black', color: 'white', padding: '10px 20px', border: 'none', borderRadius: '5px' }} 
    key = "submission"
    onClick={() => {
...
{tryAgain && (<p>Incorrect, Try again</p>)}
</div>
)
